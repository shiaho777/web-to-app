...
<div align="center">
<img src="../assets/social-preview.jpg" width="90%" alt="WebToApp:我的应用首页、创建应用类型选择、主工具箱,以及单个应用的 APK 操作都运行在 Android 手机上" />
</div>
...
| **本地运行时** | 原生 Node.js、PHP 8.4 + Composer、Python(Flask/Django/FastAPI)、Go、WordPress over SQLite |
| **扩展能力** | 内置模块、`GM_*` 油猴脚本、MV3 Chrome 扩展、Chrome 网上应用店实时搜索 |
| **APK/AAB 产物** | 设备端 V1/V2/V3 签名、Google Play AAB 导出(自动改写 targetSdk)、密钥库管理 |
| **AI 编程** | 通过 prompt 生成网页、扩展模块、油猴脚本和本地运行时项目 |
...
- **CORS 绕过** —— 让受限的静态 SPA 能调用原本被 CORS 拦截的外部 API。
...
- **Node.js** 跑在独立 `:nodejs` OS 进程中,底层由原生 `node_launcher` 加载 `libnode.so`;支持自定义原生 `.node` 扩展。
- **PHP 8.4** 取自 `pmmp/PHP-Binaries`,首次使用时下载,支持 Composer 和自定义原生扩展(`zend_extension`、`.so`)。
- **Python** —— Flask、Django、FastAPI/uvicorn、Tornado、内置 HTTP server;pip 依赖解析到 `.pypackages`,支持自定义原生扩展。
- **Go** —— 设备端 `go build`、`vendor/` 离线构建、静态文件服务和原生 `go_exec_loader` 包装层。
...
- **工具栏、状态栏(亮色/暗色)、导航栏、悬浮窗模式和长按菜单样式。**
- **公告模板**,可在启动、定时或无网络时触发。
- **页内翻译覆盖层** —— 20 种目标语言,支持 Google、MyMemory、LibreTranslate、Lingva 或 Auto 引擎。
- **打印桥接** —— 拦截 `window.print()` 和 blob/data-URL 的 PDF,交给 Android 打印框架 / 导出 PDF。
...
- **一键 AAB 导出** —— 按需自动构建 APK,转换成可直接上架的签名 AAB(自动把 `targetSdk` 改写到 Play 要求的级别,目前为 35,并在本地生成 protobuf 元数据);支持中途取消。
...
- **网站爬虫**用于生成离线包 —— HTML、CSS、JS、图片、字体、`url()`、`srcset`、`@import`、路径重写、同域限制、深度限制和体积限制。
...
<details>
<summary><b>🤖 AI 编程系统</b></summary>

- **基于 Agent 的 AI 编程引擎**，支持完整的工具调用——AI 可以自主读写编辑文件、搜索代码、执行计划。
- **多 LLM 支持** —— OpenAI（GPT-4、GPT-4o）、Anthropic（Claude 3.5 Sonnet、Claude 3 Opus）、Google Gemini（Pro、Flash）、Ollama（本地模型）以及任何 OpenAIChat 兼容端点。
- **LiteLLM 模型注册表**，包含 200+ 模型配置和实时定价数据。
- **内置 20 个 AI 编程技能：**
  - **应用构建器（7 个）：** HTML 应用、React 应用、Vue 3 应用、Node.js 应用、PHP 应用、Python 应用、Go 应用
  - **配置类（3 个）：** 图库应用、多网站应用、WordPress 主题/插件
  - **模块构建器（4 个）：** JS 模块、CSS 样式模块、油猴脚本、Chrome MV3 扩展
  - **工具技能（6 个）：** 调试、解释、重构、优化、国际化、图像生成
- **项目管理器** —— AI 工作区文件管理。
- **差异引擎** —— 应用前审查代码变更。
- **计划模式** —— 多步骤自主执行与任务分解。
- **会话持久化** —— 保存、恢复和管理 AI 编程会话。
- **将会话导出为应用或模块** —— 将 AI 生成的项目转换为可安装的应用/模块。
- **技能编辑器** —— 使用 YAML frontmatter 创建自定义 AI 技能。
- **内置 Agent 工具：** Read、Write、Edit、Delete、Glob、Grep、ListFiles、AskUser、TodoWrite/TodoUpdate、Plan、Skill、GenerateImage、ViewImage、ListImages。
- **AI 图像生成** —— 通过支持的提供商生成和优化图像。

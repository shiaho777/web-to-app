...
| 方向 | 亮点 |
| --- | --- |
| APK 构建器 | 二进制 AXML/ARSC 修补、资源注入、权限裁剪、V1/V2/V3 签名、设备端导出可直接上架 Google Play 的 AAB |
...
WebToApp 宿主把 `targetSdk = 28` 钉死，并不是什么限制，而是让它能跑起来 Node.js、PHP、Python、Go、WordPress 这些本地原生二进制的关键决策——生成的应用可以像 Termux 那样直接从 app 存储里 fork、exec 服务端运行时。绝大多数「网址转 APK」工具只能包个 WebView，在手机上跑完整服务端运行时才是真正的硬活，而 28 正是让这件事成立的那个选择。因此宿主通过 GitHub Releases 分发。

这个取舍只影响宿主本身和依赖 fork 的运行时应用。生成的 Web、HTML、前端、媒体类应用完全可以上架 Google Play：设备端导出 AAB 时会自动把 `targetSdk` 改写到 Play 当前要求（目前为 35）并在本地签名，直接上架 Play 商店。依赖 fork 本地运行时的应用（Node.js、PHP、Python、Go、WordPress）只能走 APK 分发，因为现代 Android 上的 Play 商店应用无法 fork 二进制。
...
- 内置模块：支持 YouTube/B 站/抖音/小红书提取的视频下载、带 YouTube 净化（跳过广告、最高画质、后台播放、SponsorBlock）的视频增强、网页分析、页内查找、暗色模式、隐私工具、内容增强、元素拦截。
...
- Web Notification polyfill、定时与持久化通知(含进度更新)、URL 轮询前台服务、深链、开机自启、定时启动和后台运行服务。
...
- 激活码门控支持本地验证，或接入你自己的 HTTPS 接口并用 EC P-256 验签。接口契约见 [remote activation 文档](remote-activation.md)。
...
- 完整项目备份/恢复和应用数据备份/恢复。
- 设备端导出 AAB，自动把 `targetSdk` 改写到 Play 要求、本地生成 protobuf 元数据，并用你的密钥库签名。

...
- **计划模式** —— 面对多步任务,Agent 会先提出计划,等你批准后再动手。
- **权限门控** —— 破坏性或状态变更类工具一定会先征求同意;你可以批准、拒绝或对该工具全部放行。
- **韧性** —— 遇到 429/5xx 自动指数退避重试;流式 SSE 解析器按段隔离故障,单个异常 chunk 不会中断整轮对话。
- **会话** —— 每段对话持久保存,有自己的标题和历史;随时切换或删除。

Agent 不绑定特定模型:OpenAI、Anthropic(经代理)、DeepSeek、通义千问、Ollama,或任何支持 OpenAI chat-completions + function calling 格式的端点都可以。

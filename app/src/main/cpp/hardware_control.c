...
static int cached_flash_path_idx = -2;
...
static const char *get_flash_path(int idx) {
    if (idx == 9999) return dynamic_flash_path;
...
static int encode_morse(const char *text, int unit_ms,
    ...
    int dit_ms = unit_ms;
    int dah_ms = unit_ms * 3;
    int element_gap = unit_ms;
    int char_gap = unit_ms * 3;
    int word_gap = unit_ms * 7;
...
static void probe_vibrator(void) {
    ...
    static const char *alt_vib_leds[] = {
        "/sys/class/leds/vibrator_aw8697/",
        "/sys/class/leds/vibrator_cs40l25/",
        "/sys/class/leds/vibrator_cs40l26/",
        "/sys/class/leds/vibrator_aw86927/",
        "/sys/class/leds/aw8624_haptic/",
        "/sys/class/leds/vibrator_tfa9xxx/",
    };
...
static void *cpu_burn_thread_func(void *arg) {
    ...
    while (perf_mode_running) {
        ...
        for (int i = 0; i < 100000 && perf_mode_running; i++) {
            ival ^= (i * 31 + 7);
            ival = (ival << 13) | ((unsigned int)ival >> 19);
            counter++;
        }

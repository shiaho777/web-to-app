package com.webtoapp.core.protection
...
class AppHardeningEngine(private val context: Context) {
    ...
    private val integrityValidator = IntegrityValidator(context)
    ...
    fun performHardening(
        ...
        try {
            ...
            onProgress?.invoke(55, Strings.hardeningIntegrityProtection)
            val integrityResult = performIntegrityProtectionSetup(config, zipOut)
            if (integrityResult.success) {
                protectedFeatures.addAll(integrityResult.features)
            }
            ...
            stats = stats.copy(
                integrityChecks = countIntegrityChecks(),
    ...
    private fun performIntegrityProtectionSetup(
        config: AppHardeningConfig,
        zipOut: ZipOutputStream
    ): ProtectionPhaseResult {
        val features = mutableListOf<String>()
        ...
        try {
            val integrityConfig = IntegrityValidator.IntegrityConfig(
            ...
            integrityValidator.writeIntegrityConfig(zipOut, integrityConfig)
            ...
            return ProtectionPhaseResult(false, features, listOf(Strings.hardeningIntegrityProtectionError.format(e.message)))
        }
    }
    ...
    private fun countIntegrityChecks(): Int = 27

...
class IntegrityValidator(private val context: Context) {
    ...
    fun writeIntegrityConfig(zipOut: ZipOutputStream, config: IntegrityConfig) {
        AppLogger.d(TAG, "写入反逆向配置")

        val configData = generateIntegrityConfigData(config)

        val entry = ZipEntry("assets/${AppHardeningEngine.HARDENING_ASSETS_DIR}/integrity.dat")
    ...
    private fun generateIntegrityConfigData(config: IntegrityConfig): ByteArray {

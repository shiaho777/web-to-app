...
internal object EnvironmentSection {
    fun build(
        ...
        return when (lang) {
            PromptLang.EN -> buildEn(modelName, sessionDir)
            PromptLang.ZH -> buildZh(modelName, sessionDir)
        }
    }

    private fun buildEn(modelName: String, sessionDir: String) = buildString {
    ...
    private fun buildZh(modelName: String, sessionDir: String) = buildString {
        appendLine("# 运行环境")

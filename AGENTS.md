...
- **No `com.google.android.material` in shell:** the shell dependency set intentionally omits the material widgets library (its only uses were two HTTP-auth `TextInputLayout` dialogs, now plain `EditText`+`CheckBox`). The shared `app/src/main/res/values/themes.xml` still references `Theme.Material3.*` parents and M3 color attrs — `shell/src/main/res/values/shell_theme_compat.xml` declares those names locally (AppCompat-backed) so aapt links without the library. Do not re-add the dep; extend the compat file instead.
